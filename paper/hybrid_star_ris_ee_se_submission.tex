...
Fig. \ref{fig:pareto} shows the EE-SE Pareto frontier. At very low SE, the passive STAR-RIS remains competitive because no active hardware is needed, and at $2$ bit/s/Hz it is nearly coincident with the proposed sparse-hybrid design. As the target SE increases, however, the passive benchmark suffers a rapidly growing BS-power burden, while the fully active benchmark pays a large amplification cost. The proposed sparse-hybrid design dominates the medium-to-high SE region and stays feasible up to $20$ bit/s/Hz under the considered budget. At $12$ bit/s/Hz, the proposed design improves EE by approximately 20.5\% over uniform hybrid activation, 59.3\% over the fully active design, and 71.2\% over the passive design. This again validates the main message of the paper: only a minority of carefully chosen active elements is needed to unlock most of the SE gain.
...
Fig. \ref{fig:design} compares the protocol-dependent design rules of the proposed sparse-hybrid solver. TS requires the densest active set because the effective slot targets are inflated by $1/\tau_s$; at $12$ bit/s/Hz its active ratio is about $0.69$, versus about $0.30$ for ES and $0.36$ for SS. ES stays between the two extremes, whereas SS tracks ES more closely than TS over the whole range while avoiding the severe slot-rate penalty. The gain curves follow the same regime split: TS reaches the gain cap earlier, whereas ES and SS increase more gradually before all three protocols saturate near the high-SE end.
...
Fig. \ref{fig:noise_dyn} compares the hardware sensitivity across ES, TS, and SS at the representative target $R_\Sigma=12$ bit/s/Hz. In Fig. \ref{fig:noise_dyn}(a), the average side-wise gain decreases with $\sigma_a$ for all three protocols, with the strongest drop appearing under TS because the slot-rate inflation already forces a denser active set. ES and SS remain close to the gain cap over a broader noise range, while TS backs off its preferred gain much earlier. Fig. \ref{fig:noise_dyn}(b) shows that increasing the state-update frequency from $50$ Hz to $550$ Hz reduces EE for all three protocols; ES stays slightly above SS over most of the range, and TS stays consistently lower because its denser activation pattern magnifies the dynamic-cost penalty.
