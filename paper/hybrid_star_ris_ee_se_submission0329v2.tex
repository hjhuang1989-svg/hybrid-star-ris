...
\begin{abstract}
Hybrid active-passive simultaneously transmitting and reflecting reconfigurable intelligent surfaces (STAR-RISs) can improve spectral efficiency (SE) relative to fully passive architectures. Their energy efficiency (EE) behavior, however, remains insufficiently understood because existing analyses often rely on simplified amplifier-noise models, simplified control and switching power models, or asymmetric task models for the transmission and reflection sides. This paper develops a tractable analytical framework for characterizing the EE-SE Pareto boundary of hybrid active-passive STAR-RISs. We first introduce a three-part power model that accounts for static controller and bias power, dynamic switching overhead, and communication-side transmit and amplification power. For a sparse active set, we derive a closed-form SE-optimal amplification structure and an active-on threshold that quantifies when amplification gains offset amplifier noise. We then characterize the Pareto boundary via an $\epsilon$-constraint formulation with a low-complexity sparse-activation solver and show that the resulting coefficient rules extend across ES, TS, and SS modes. Numerical results for the studied settings show that the proposed sparse-hybrid design improves EE over the baseline schemes.
\end{abstract}
...
Simultaneously transmitting and reflecting reconfigurable intelligent surfaces (STAR-RISs) extend passive beam manipulation to simultaneous transmission and reflection, allowing a common surface to serve transmission-side and reflection-side users \cite{liu2021star,mu2022star}. This capability is attractive for asymmetric indoor-outdoor coverage. However, purely passive STAR-RIS still suffers from multiplicative path loss \cite{wu2019irs}.
...
\begin{figure}[t]
	...
	\label{fig:system}
\end{figure}
However, existing hybrid active-passive STAR-RIS studies do not clearly specify when the SE gain of an active element offsets its associated noise and power cost under given channel and hardware conditions \cite{vu2025hybridstar}. Related active/passive STAR-RIS studies address neighboring settings such as SWIPT, URLLC, and ISAC \cite{zhu2025activepassive,soleymani2023starurlcc,zhang2024activestarisac}. To address this gap, we develop an equivalent formulation for hybrid STAR-RIS and derive tractable Pareto-boundary characterizations. The main contributions are as follows.
\begin{itemize}
    ...
    \item We develop a sparse-activation $\epsilon$-constraint solver consistent with the analytical model and show that its coefficient rules extend across ES, TS, and SS. Simulations indicate that passive STAR-RIS is most competitive at low SE, whereas sparse hybrid activation yields higher EE at moderate-to-high SE under the considered settings.
\end{itemize}
...
\end{equation}
We further assume equal per-user rate splitting within each side. This captures side-wise asymmetric traffic demands but not arbitrary user-specific QoS.
...
\label{eq:threshold}
\end{equation}
Equation \eqref{eq:threshold} specifies when active amplification is beneficial: the coherent-gain increase must exceed the normalized amplifier-noise penalty. At high SNR (small effective $c_s$), this condition becomes harder to satisfy.

\subsection{Power-minimizing gain and a sufficient convexity condition}
For a target side rate $R_s$, we define the cost as the sum of the minimum BS transmit power required to meet the rate target and the intrinsic power consumed by the active amplifiers:
\begin{equation}
\Psi_s(\beta_s)=
...
\end{equation}
then $\Psi_s(\beta_s)$ is strictly convex on $[1,\beta_{\max}]$. Consequently, the side-wise power-minimizing gain is unique. It can be found by checking the two boundaries and the unique root of \eqref{eq:side_derivative}.
\end{proposition}

\begin{IEEEproof}
Under \eqref{eq:convexity_cond}, the bracketed term in \eqref{eq:side_second} is non-negative on $[1,\beta_{\max}]$ and, being affine in $\beta_s$, can vanish at most once. Hence $\Psi_s''(\beta_s)$ is non-negative everywhere and positive on every nontrivial subinterval. Therefore, $\Psi_s'(\beta_s)$ is strictly increasing and $\Psi_s(\beta_s)$ is strictly convex. The unique minimum is attained at an endpoint or at the unique root of \eqref{eq:side_derivative}.
\end{IEEEproof}
...
In the numerical study, we therefore set $\beta_s=1$ in the first two cases. We solve the strictly convex scalar problem in the fully active case and use \eqref{eq:beta_closed} with the refinement of Proposition~\ref{prop:convexity} in the mixed case.

\subsection{Extension of the coefficient rules to TS and SS}
Since ES is already covered by \eqref{eq:abcd}, for $p\in\{\mathrm{TS},\mathrm{SS}\}$ let $\mathcal{P}_s^{(p)}$ and $\mathcal{A}_s^{(p)}$ denote the passive and active support sets. We then define
\begin{align*}
...
Motivated by the active-on threshold in \eqref{eq:threshold}, we propose a heuristic sparse-activation strategy. It prioritizes elements offering a high ratio of coherent signal gain to noise-and-hardware cost. For a candidate element $n\notin\mathcal{A}_s$, we introduce a relaxed inclusion variable $x_{n,s}\in[0,1]$. For fixed $R_s$, $\beta_s$, and current aggregates $(a_s,b_s,c_s,d_s)$, define
\[
...
\label{eq:score}
\end{equation}
where $\omega_s$ reflects the side demand and $\lambda_1,\lambda_2$ absorb the side-wise constants. In the simulations, $\omega_t=\alpha$, $\omega_r=1-\alpha$, $\lambda_1=\sigma_0^{-2}$, and $\lambda_2=(P_{\max}^{\mathrm{BS}})^{-1}$. Candidates are ranked by $\max_s\xi_{n,s}$ and assigned to the maximizing side. They are then re-evaluated through \eqref{eq:minsidepower}, \eqref{eq:side_cost}, and \eqref{eq:power_model}.

\subsection{Inner gain and rate-split update}
For a given active cardinality $L=|\mathcal{A}|$, we search over the 1-D rate split $R_t=\alpha\epsilon$ and $R_r=(1-\alpha)\epsilon$. After updating $\mathcal{A}_t$ and $\mathcal{A}_r$ via \eqref{eq:score}, we compute $(a_s,b_s,c_s,d_s)$ and update $\beta_s$ case by case. We set $\beta_s=1$ if $|\mathcal{A}_s|=0$ or $b_s=0$, solve $\min_{\beta_s\in[1,\beta_{\max}]}\Psi_s(\beta_s)$ if $a_s=0$, and otherwise use \eqref{eq:beta_closed} or Proposition~\ref{prop:convexity}. The best feasible $(L,\alpha)$ minimizes \eqref{eq:power_model}.
...
We consider a hybrid STAR-RIS with $N=24$ elements, $K_t=K_r=2$, Nakagami-$m$ fading, and the normalized parameters listed in Table~\ref{tab:params}. Each curve averages $100$ independent channel draws. Unless stated otherwise, the BS power budget is $5.0$ W. The ES coefficients are $\rho_t=0.55$ and $\rho_r=0.45$, the TS slot fractions are $\tau_t=0.55$ and $\tau_r=0.45$, and the gain cap is $\beta_{\max}=5$.
...
Fig.~\ref{fig:pareto} shows the EE-SE Pareto boundary. Passive STAR-RIS is most competitive at very low SE, whereas the fully active benchmark incurs higher amplification cost as the target SE increases. Among the considered schemes, the proposed sparse-hybrid design achieves the highest EE in the medium-to-high-SE region and remains feasible up to $20$ bit/s/Hz. At $12$ bit/s/Hz, the EE gain is approximately 30.7\% over uniform hybrid activation, 74.9\% over the fully active design, and 94.4\% over the passive design.
...
Fig.~\ref{fig:breakdown} compares the ES power decomposition at $R_\Sigma=12$ bit/s/Hz. The fully active design minimizes the BS transmit-power share, about $0.38$ W on average, but also incurs the largest static/dynamic and amplification costs, with the amplification term alone reaching about $1.44$ W. The passive design avoids amplification but requires about $2.78$ W of BS power. The proposed sparse-hybrid scheme achieves the lowest total power, about $1.44$ W versus about $1.91$ W for uniform hybrid, by reducing the BS radiated-power requirement while keeping the amplification term well below the fully active benchmark.
...
\section{Conclusion}
This paper studied the EE-SE tradeoff of hybrid active-passive STAR-RIS through a structured Pareto-boundary formulation linking amplifier noise, active-set sparsity, side-wise asymmetry, and hardware power. The analysis yielded a closed-form SE-optimal gain, an active-on threshold, and a sparse-activation rule. The $\epsilon$-constraint solver translated these results into a reproducible boundary generator. Numerical results indicated that passive STAR-RIS is most competitive at low SE, whereas sparse hybrid activation yields higher EE at moderate-to-high SE under the considered settings. TS requires denser activation and is more sensitive to amplifier noise than ES and SS. Extending the framework to CSI-imperfect or calibration-aware designs remains future work.
