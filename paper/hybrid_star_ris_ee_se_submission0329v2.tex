...
Simultaneously transmitting and reflecting reconfigurable intelligent surfaces (STAR-RISs) extend passive beam manipulation to simultaneous transmission and reflection, allowing a common surface to serve transmission-side and reflection-side users \cite{liu2021star,mu2022star}. This capability is attractive for asymmetric indoor-outdoor coverage, but purely passive STAR-RIS still suffers from multiplicative path loss \cite{wu2019irs}.
...
\begin{figure}[t]
	...
	\label{fig:system}
\end{figure}
However, existing hybrid active-passive STAR-RIS studies still leave one critical architectural question unresolved: under what exact channel and hardware conditions does the SE gain of an active element offset its intrinsic noise and power penalty \cite{vu2025hybridstar}? Related active/passive STAR-RIS studies address neighboring settings such as SWIPT, URLLC, and ISAC \cite{zhu2025activepassive,soleymani2023starurlcc,zhang2024activestarisac}. To answer this question, we develop an equivalent formulation for hybrid STAR-RIS and derive tractable Pareto-boundary characterizations. The main contributions are as follows.
\begin{itemize}
...
Motivated by the analytical insights from the active-on threshold in \eqref{eq:threshold}, we propose a heuristic sparse-activation strategy that prioritizes elements offering a high ratio of coherent signal gain to noise-and-hardware cost. For a candidate element $n\notin\mathcal{A}_s$, introduce a relaxed inclusion variable $x_{n,s}\in[0,1]$. For fixed $R_s$, $\beta_s$, and current aggregates $(a_s,b_s,c_s,d_s)$, define
\[
